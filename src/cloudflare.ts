...
// Phase of the ruleset holding the zone's custom WAF rules.
const firewallPhase: string = "http_request_firewall_custom"
...
export const getFirewallId = async (): Promise<string> => {
    try {
        const expression = `ip.src in $${settings.cloudflare.listName}`
        const ruleset = await getFirewallRuleset()
        const rules: any[] = ruleset?.rules || []
        const rule = rules.find((i) => i.action == "skip" && i.expression?.includes(expression))

        if (rule) {
            logger.info("Cloudflare.getFirewallId", rule.id)
            return rule.id
        } else {
            const ruleId = await createFirewall(ruleset?.id)
 ...
 * @param rulesetId ID of the zone's custom rules ruleset, if it already exists.
 */
export const createFirewall = async (rulesetId?: string): Promise<string> => {
    try {
        const expression = `ip.src in $${settings.cloudflare.listName}`
        const rule: any = {
            action: "skip",
            ...
            description: "Generated by AllowMe",
            enabled: true
        }
        ...
        if (rulesetId) {
            ...
            res = await makeRequest(`zones/${settings.cloudflare.zoneId}/rulesets/${rulesetId}/rules`, "POST", rule)
        } else {
            const data = {name: "AllowMe", kind: "zone", phase: firewallPhase, rules: [rule]}
            res = await makeRequest(`zones/${settings.cloudflare.zoneId}/rulesets`, "POST", data)
        }

        const created = (res?.rules || []).find((i) => i.action == "skip" && i.expression?.includes(expression))
        if (!created) {
            throw new Error(`Firewall rule was not created${rulesetId ? " for ruleset " + rulesetId : ""}`)
        }

        logger.info("Cloudflare.createFirewall", created.id)
        return created.id
    } catch (ex) {
...
/**
 * Get the zone's custom rules ruleset, or null if it doesn't exist yet.
 */
const getFirewallRuleset = async (): Promise<any> => {
    try {
        return await makeRequest(`zones/${settings.cloudflare.zoneId}/rulesets/phases/${firewallPhase}/entrypoint`)
    } catch (ex) {
        if (ex.statusCode == 404) {
            return null
        }
        logger.error("Cloudflare.getFirewallRuleset", ex)
        throw ex
    }
}
...
const makeRequest = async (path: string, method?: "GET" | "POST" | "DELETE", body?: any): Promise<any> => {
    try {
        ...
        if (ex.response) {
            ...
            const err: any = new Error(`${ex.response.status} - ${details}`)
